...
import { fetchContestsRequest, joinContestRequest, resetSuccessMessage } from '../../redux/actions/contestActions';
import {
  Box,
  Tab,
  Tabs,
...
const Contests = () => {
  ...
  const { contests, loading, error, successMessage } = useSelector(state => state.contestData);
  ...
  const getListData = () => {
    if (searchTxt) {
      const dataList =
        (selectedModule === 0 && contests.data?.liveContests) ||
        (selectedModule === 1 && contests.data?.expiredContests) ||
        contests.data?.upcomingContests;
      const filteredData = dataList.filter(item =>
        item.title.toUpperCase().includes(searchTxt.toUpperCase())
      );
      return filteredData;
    } else {
      if (selectedModule === 0) return contests.data?.liveContests;
      if (selectedModule === 1) return contests.data?.expiredContests;
      if (selectedModule === 2) return contests.data?.upcomingContests;
    }
  };
  useEffect(() => {
  ...
  const joinTheContest = (contestId,contestActionType) => {
    dispatch(joinContestRequest(contestId, contestActionType));
  };
  ...
  return (
    <div className="contests-container">
      <Tabs
        ...
        centered
      >
        <Tab label="Live Contests" className='tab-nav-contest' />
        <Tab label="Expired Contests" className='tab-nav-contest' />
        <Tab label="Upcoming Contests" className='tab-nav-contest' />
      </Tabs>
      <br />
        <>
          <p className="tip-txt">Get Creative with special contest & Win Amazing Prizes!</p>
          <input
            type="text"
